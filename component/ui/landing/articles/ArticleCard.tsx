...
export default function ArticleCard({
    ...
    return (
        <div className={"flex flex-col gap-4 bg-teal-50 rounded-xl p-4 items-center relative mb-12 shadow shadow-slate-400"}>
            ...
            <div className={"absolute -bottom-6 flex justify-center gap-4 bg-white w-fit rounded-full"}>
                <Button buttonType={"button"} variant={"success"} variantType={"solid-rounded"} buttonName={"cta"} buttonText={"Read More"} size={"md"} icon={<MdReadMore className={"size-6"}/>} customClass={"flex-row-reverse"}/>
            </div>
        </div>
    );
}